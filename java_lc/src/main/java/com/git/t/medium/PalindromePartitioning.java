...
public class PalindromePartitioning {
    ...
    private void partitionHelperDp(boolean[][] dp, int start, List<List<String>> response, char[] toCharArray, ArrayList<String> current) {
        if (start >= toCharArray.length)
            response.add(new ArrayList<>(current));
        for(int end=start ; end <toCharArray.length;end++){
            if((toCharArray[start]== toCharArray[end])&&(end-start<=2||dp[start+1][end-1])){
                dp[start][end]=true;
    ...
    private void partitionHelper(int start, List<List<String>> response, char[] charSeq, ArrayList<String> current) {
