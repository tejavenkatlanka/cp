...
public class BuildTreeFromPreAndInOrder {
  ...
  private TreeNode buildTreeHelper(int preStart, int inStart, int inEnd, int[] preorder,
      int[] inorder) {
    ...
    node.right = buildTreeHelper(preStart + index - inStart + 1, index + 1, inEnd, preorder,
        inorder);
    return node;
  }
}
